...
这是当前极简邀请房平台的统一语言。当前支持五子棋、中国象棋、井字棋、挑夹棋、警察抓小偷、扫雷（单人/双人竞速）、本地多人跳棋、单机 2048 与单机贪吃蛇；以后增加其他棋类时保持房间基础设施不变。首页展示六个房间/实时棋类入口，以及跳棋、2048、贪吃蛇三个本地入口；扫雷在统一选择层中再选单人/双人竞速和难度。
...
**Room Capacity（房间容量）**
平台允许同时存在的尚未废弃 Room 数量；当前上限为 10。跳棋、单人扫雷、2048、贪吃蛇等本地游戏和全局排行榜 Durable Object 不是 Room，不占用名额。
