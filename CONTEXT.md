...
规则集为 `minesweeper.race.*.v1`。服务端生成一张权威雷区，双方从同一中央安全起点开始，但各自保存独立的 revealed 和 flags。终局前对手和观众只看到双方进度数，不看到对手格子坐标；先揭开全部安全格者获胜，踩雷者立即失败。旧 `minesweeper.duel.*.v1` 仅保留协议兼容，新建房入口使用 Race 规则。

**Minesweeper Leaderboard（扫雷排行榜）**
单人扫雷按小型、中型、大型分榜，显示当前签名 Guest 的个人最佳和全站 Top 10。每个 Guest 每个难度只保留最快成绩，榜单保存签名会话中的 Display Name 和用时，不公开 Guest ID。它是休闲榜，没有账号或完整服务端回放校验，不宣称强反作弊。
