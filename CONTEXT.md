...
**Sokoban Solo（推箱子单机）**
在浏览器内运行不可变规则版本 `sokoban.microban-1-10.v1`，不创建 Room、不占用房间名额。首批内置 David W. Skinner 创作的 Microban 第 1–10 关，关卡文本按原作者“可自由转载但须署名”的许可保留可见署名和固定 GitHub 来源；规则引擎把墙、外部空白、地板、目标点与动态箱子/玩家分离，页面支持方向键、WASD、滑动、屏幕方向按钮、撤销、重开和关卡直达参数 `?level=1..10`。后续关卡继续追加到独立关卡目录，不改变移动接口。
