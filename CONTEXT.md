...
**2048 Solo（2048 单机）**
固定 4×4 网格的本地游戏，在浏览器内维护局面；不创建 Room、不占用房间名额。达到 2048 后仍可继续合并，棋盘无法继续移动时本局结束并提交分数。
...
全站只设一个榜，显示当前签名 Guest 的个人最高分和全站 Top 10。每个签名 Guest 只保留一条个人最高分，较低或同分成绩不替换；榜单按 `score` 降序，分数相同时再按 `achieved_at`、`guest_id` 升序确定性排序。榜单保存签名会话中的 Display Name 和分数，不公开 Guest ID；成绩绑定不可变的 `2048.solo.4x4.v1` 规则版本，服务端按写入时间最多保留 180 天并每日清理。分数由客户端提交，接口虽有限流和输入校验，但没有账号或完整服务端回放校验，因此只定位为休闲榜，不宣称强反作弊。
