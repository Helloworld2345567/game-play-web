...
**Action Journal（动作日志）**
并发规则的有界幂等回执表。客户端在单个连接内单调生成 `clientSeq`；服务端按 Seat 将 `actionId + clientSeq` 作为有限窗口去重键，允许多个连接的动作乱序到达，最多保留最近 128 条可见回执，并记录已经淘汰到哪个序号；落在淘汰下界内的动作返回 `room.action_expired`，同一序号换用其他 ID 返回 `room.action_sequence_conflict`，两者都不得再次进入规则模块。拒绝回执可以单独持久化，但不推进游戏或快照修订号，也不触发整房广播。
