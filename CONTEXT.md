...
这是当前极简邀请房平台的统一语言。当前支持五子棋、中国象棋、井字棋、挑夹棋、警察抓小偷、扫雷（单人/双人竞速）、2/3/4 人联机跳棋、单机 2048、单机贪吃蛇与单机推箱子；以后增加其他棋类时保持房间基础设施不变。首页展示七个房间/实时棋类入口，以及 2048、贪吃蛇、推箱子三个本地入口；扫雷在统一选择层中再选单人/双人竞速和难度。
...
**Room Capacity（房间容量）**
平台允许同时存在的尚未废弃 Room 数量；当前上限为 10。新建房间先持有 60 秒 provisional 租约，只有首个 WebSocket 或 HTTPS sync 成功后才计入 `activeRooms`，但 provisional 租约同样占用容量并在超时或明确废弃时释放。单人扫雷、2048、贪吃蛇、推箱子等本地游戏和全局排行榜 Durable Object 不是 Room，不占用名额；跳棋联机房间计入房间上限。
