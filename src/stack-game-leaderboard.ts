...
import type { StackGameLeaderboardSnapshot } from "./shared/game-stack-leaderboard";

export const STACK_GAME_LEADERBOARD_NAME = "global-stack-game-leaderboard-v1";
export const STACK_GAME_LEADERBOARD_RETENTION_MS = 180 * 24 * 60 * 60_000;
...
export {
  STACK_GAME_MAX_SCORE,
  type StackGameLeaderboardEntry,
...
function assertRuleVersion(value: unknown): asserts value is StackGameRuleVersion {
  if (!isStackGameRuleVersion(value)) {
    throw new TypeError("Invalid Stack Game rule version");
  }
}

/**
 * Global casual leaderboard for the versioned Stack Game rules.
 *
 * The DO stores one personal best per signed Guest and rule version.  Guest
 * identifiers and timestamps are used only for deterministic ordering and are
 * intentionally omitted from the public snapshot.
 */
export class StackGameLeaderboard extends DurableObject<EmptyEnv> {
  constructor(ctx: DurableObjectState, env: EmptyEnv) {
    super(ctx, env);
    this.ensureSchema();
    this.ctx.storage.sql.exec(`
      CREATE INDEX IF NOT EXISTS stack_game_personal_bests_ranking
  ...
  private pruneExpired(now: number): void {
    this.ctx.storage.sql.exec(
      ...
      now - STACK_GAME_LEADERBOARD_RETENTION_MS,
    );
  }
  ...
  private ensureSchema(): void {
    ...
    if (
      ...
      throw new Error("Unsupported Stack Game leaderboard schema");
    }
  }

  private createPersonalBestsTable(): void {
    this.ctx.storage.sql.exec(`
