...
一个部署在 Cloudflare 上的极简实时棋类对战平台。首个棋种是 15×15 自由五子棋：无需注册，创建房间后把邀请链接发给朋友即可开始。
...
同一个 TypeScript 项目构建 Preact 静态页面与 Cloudflare Worker。Worker 负责会话和路由；`GameRoom` Durable Object 串行处理一个房间内的命令、持久化状态并广播快照。棋种规则通过 `GameRules` 注册表隔离，前端通过对应的 `GameAdapter` 注册表选择棋盘，因此增加象棋不需要修改房间、会话或重连核心。
...
测试覆盖纯规则/房间状态、真实 workerd Durable Object 与 WebSocket 集成，以及两个独立浏览器身份的邀请、断网恢复、完整胜局、复赛和第三人房满流程。
...
房间协议只传递不透明的棋种 payload，平台核心不读取棋种私有局面。
