...
import {
  ...
  STACK_GAME_LEADERBOARD_RETENTION_MS,
  type StackGameLeaderboard,
} from "../src/stack-game-leaderboard";
import {
...
function leaderboard(): DurableObjectStub<StackGameLeaderboard> {
  const testEnv = env as unknown as TestEnv;
  return testEnv.STACK_GAME_LEADERBOARD.getByName(STACK_GAME_LEADERBOARD_NAME);
}
...
describe("StackGameLeaderboard Durable Object", () => {
  it("records a first score and returns only the public leaderboard view", async () => {
    const stub = leaderboard();

    await expect(
      stub.snapshot(STACK_GAME_SOLO_RULE_VERSION, "guest-alice"),
    ).resolves.toEqual({
  ...
  it("accepts the inclusive valid score boundaries", async () => {
    ...
    await expect(
      stub.snapshot(STACK_GAME_SOLO_RULE_VERSION, "guest-slow"),
    ).resolves.toEqual({
      ruleVersion: STACK_GAME_SOLO_RULE_VERSION,
      personalBestScore: STACK_GAME_MAX_SCORE,
      top: [
        { rank: 1, displayName: "最高", score: STACK_GAME_MAX_SCORE },
  ...
  it("rejects malformed Guest identifiers, names, and rule versions", async () => {
    ...
    await expect(
      rejectionMessage(
        stub.snapshot(STACK_GAME_SOLO_RULE_VERSION, "bad guest"),
      ),
    ...
    await expect(
      rejectionMessage(stub.snapshot("stack-game.solo.v0", "guest-alice")),
    ).resolves.toBe("Invalid Stack Game rule version");
  });
  ...
  it("persists scores across Durable Object restarts", async () => {
    ...
    await expect(
      restartedStub.snapshot(STACK_GAME_SOLO_RULE_VERSION, "guest-alice"),
    ).resolves.toEqual({
      ruleVersion: STACK_GAME_SOLO_RULE_VERSION,
  ...
  it("removes records once the documented retention window has elapsed", async () => {
    ...
    clock.mockReturnValue(1_000 + STACK_GAME_LEADERBOARD_RETENTION_MS + 1);
