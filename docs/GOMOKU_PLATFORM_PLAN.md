...
将 `https://play.ym0v0.com` 部署为一个 Cloudflare Worker：它同时提供前端静态资源、少量 HTTP API 和 WebSocket；每个房间对应一个 SQLite-backed Durable Object，串行裁决并保存完整棋局。除此之外不启用任何 Cloudflare 数据服务。
...
“最多 10 人”按容量假设理解，即最多约 5 个同时对局，不做全站硬人数闸门。若以后必须严格限制在线人数，才增加一个全局 Admission DO；目前增加它只会制造断线计数和单点复杂度。
...
明确延后：账号、D1、永久棋谱、回放、排位、Elo、排行榜、赛季、快速匹配、棋钟、AI、观战、聊天、举报、管理后台、PWA、KV、R2、Queues 和支付。首版没有账号与历史，因此浏览器数据清除后不承诺恢复旧席位。
...
1. 页面启动先调用 `POST /api/session`。Worker 验证并复用已有匿名 Cookie；仅在缺失或签名无效时生成高熵匿名 ID。Cookie 使用 HMAC 签名，设置 `Secure; HttpOnly; SameSite=Lax; Max-Age=2592000`，30 天有效期覆盖房间保留期。
...
4. 创建者固定为 Seat A；邀请链接只允许第一个不同的匿名身份占 Seat B，之后的不同身份收到房间已满。相同身份刷新或多标签页仍回到原席位。
...
五子棋完整快照通常只有几 KB；最多 10 个连接时，全量同步比 delta、事件日志、补包和命令去重缓存更简单可靠。客户端断线后指数退避重连，离线时禁用操作，重连以服务端 snapshot 覆盖本地状态。
...
- 正常落子只执行一次小对象读取/写入和一次房间广播，不访问其他服务。首次创建 DO 使用 `locationHint: "apac"`。
...
- 昵称可选，规范化后最多 20 个字符；渲染为文本，不解析 HTML。
...
- 小型容量测试同时建立 10 个连接、运行 5 局，不应出现错误、顺序错乱或明显 ACK 延迟增长。
